 ...
 */
//@formatter:off
public class RefundVipCard {
  public static final int FROM_CHECKOUT_FRAGMENT = 1;
  public static final int FROM_MESSAGES = 2;

  public static void refundVipCard(final int from, final Activity act, final PxPayInfo payInfo,
 ...
 // req.setSelfTradeNo(tradeNo);
    final MaterialDialog refundDialog = DialogUtils.showDialog(act, "会员卡退款", "退款中,请耐心等待!");
    //加锁
    ...
    new RestClient(0, 1000, 10000, 5000) {
      ...
      @Override protected void failure(String responseString, Throwable throwable) {
        ...
        if (throwable instanceof SocketTimeoutException) {// TODO 服务器响应超时 需在查询
          ...
          showContinueDialog(act,tradeNo,payInfo,payInfoAdapter,pos,from,dbOrder);
        } else { //
    ...
    }.postOther(app, URLConstants.VIP_CARD_CONSUME_RECORD_REVERSE, req);
  }
   ...
   */
  //@formatter:off
  private static void showContinueDialog(final Activity act, final String tradeNo, final PxPayInfo payInfo,
    ...
    posBtn.setOnClickListener(new View.OnClickListener() {
      @Override public void onClick(View v) {
       DialogUtils.dismissDialog(queryDialog);
        continueQueryResult(act,tradeNo,payInfo,payInfoAdapter,pos,from,orderInfo);
      }
    });
   ...
   * 继续查询 退款结果
   */
  private static void continueQueryResult(final Activity act, final String tradeNo,
      final PxPayInfo payInfo, final PayInfoAdapter payInfoAdapter, final int pos, final int from,
    ...
    req.setTradeNo(tradeNo);
    req.setCid(cid);
    final MaterialDialog refundQueryDialog = DialogUtils.showDialog(act, "会员卡退款", "查询中,请耐心等待!");
    ...
    new RestClient(0, 1000, 10000, 5000) {
      ...
      @Override protected void failure(String responseString, Throwable throwable) {
        ...
        if (throwable instanceof SocketTimeoutException) {// TODO 服务器响应超时 需在查询
          showContinueDialog(act, tradeNo, payInfo, payInfoAdapter, pos, from, orderInfo);
        } else { //
          ToastUtils.showShort(app, "连接超时,请检查网络连接!");
          //释放
          OptOrderLock.optOrderLock(orderInfo, false);
  ...
  private static void dealOrderIsUseVip(PxPayInfo payInfo) {
    ...
    queryBuilder.where(PxPayInfoDao.Properties.PxOrderInfoId.eq(id));
   ...
   * 恢复商品折扣率
   */
  private static void cleanProductDiscount(Long id) {
    //恢复商品详情状态
    List<PxOrderDetails> orderDetailsList = DaoServiceUtil.getOrderDetailsService()
        .queryBuilder()
        .where(PxOrderDetailsDao.Properties.PxOrderInfoId.eq(id))
        .list();
    ...
    List<PxExtraDetails> extraDetailsList = DaoServiceUtil.getExtraDetailsService()
        .queryBuilder()
        .where(PxExtraDetailsDao.Properties.PxOrderInfoId.eq(id))
        .list();
