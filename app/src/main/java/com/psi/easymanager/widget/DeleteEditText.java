...
public class DeleteEditText extends EditText implements View.OnFocusChangeListener, TextWatcher {
  ...
  private void setDrawable(Context context) {
    ...
    if (mLeftDrawable == null) {
      mLeftDrawable = getResources().getDrawable(R.mipmap.ic_reserve_search);
    }
    mLeftDrawable.setBounds(0, 0, px, px);
  }

  // 更新删除图片状态: 当内容不为空，而且获得焦点，才显示右侧删除按钮
  private void updateDrawable(boolean hasFocus) {
    if (length() > 0 && hasFocus) {
      setCompoundDrawables(mLeftDrawable, getCompoundDrawables()[DRAWABLE_TOP], mClearDrawable,
          getCompoundDrawables()[DRAWABLE_BOTTOM]);
    } else {
      setCompoundDrawables(mLeftDrawable, getCompoundDrawables()[DRAWABLE_TOP], null,
          getCompoundDrawables()[DRAWABLE_BOTTOM]);
    }
  }
